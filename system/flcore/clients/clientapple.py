    ...
    def train(self, R):
        ...
        for step in range(max_local_epochs):
            for i, (x, y) in enumerate(trainloader):
                ...
                for param_c, param in zip(
                    self.model_cs[self.id].parameters(), self.model.parameters()
                ):
                    param_c.data = (
                        param_c - self.learning_rate * param.grad * self.ps[self.id]
                    )
                ...
                for cid in range(self.num_clients):
                    ...
                    p_grad = 0
                    for param_c, param in zip(
                        self.model_cs[cid].parameters(), self.model.parameters()
                    ):
                        p_grad += torch.mean(param.grad * param_c).item()
                        cnt += 1
                    p_grad = p_grad / cnt
                    p_grad = p_grad + self.lamda * self.mu * (
                        self.ps[cid] - self.p0[cid]
                    )
